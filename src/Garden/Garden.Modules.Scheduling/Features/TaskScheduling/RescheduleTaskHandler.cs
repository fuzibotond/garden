...
public class RescheduleTaskHandler
{
    private readonly GardenDbContext _dbContext;
    private readonly ICurrentUser _currentUser;

    public RescheduleTaskHandler(GardenDbContext dbContext, ICurrentUser currentUser)
    {
        _dbContext = dbContext;
        _currentUser = currentUser;
    }

    public async Task<TaskScheduleDto> Handle(RescheduleTaskRequest request)
    {
        if (!_currentUser.IsAuthenticated || _currentUser.UserId is null)
            throw new UnauthorizedAccessException("User is not authenticated.");

        if (!_currentUser.Roles.Contains("Gardener"))
            throw new UnauthorizedAccessException("Only gardeners can reschedule tasks.");

        var gardenerId = _currentUser.UserId.Value;

        // Verify rescheduled time is in the future
        if (request.RescheduledAtUtc <= DateTime.UtcNow)
        ...
        if (schedule.GardenerId != gardenerId)
            throw new UnauthorizedAccessException("Gardener can only reschedule their own schedule requests.");

        if (schedule.Status != TaskScheduleStatus.Pending 
            && schedule.Status != TaskScheduleStatus.ProposedAlternative 
            && schedule.Status != TaskScheduleStatus.Declined)
            throw new InvalidOperationException($"Cannot reschedule a schedule with status {schedule.Status}.");

        schedule.Status = TaskScheduleStatus.Rescheduled;
        schedule.ScheduledAtUtc = request.RescheduledAtUtc;
        schedule.ProposedAtUtc = null;

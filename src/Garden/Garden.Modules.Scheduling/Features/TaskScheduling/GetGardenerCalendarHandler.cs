...
public class GetGardenerCalendarHandler
{
    private readonly GardenDbContext _dbContext;
    private readonly ICurrentUser _currentUser;

    public GetGardenerCalendarHandler(GardenDbContext dbContext, ICurrentUser currentUser)
    {
    ...
    public async Task<GetCalendarResponse> Handle(int page = 1, int pageSize = 20)
    {
        ...
        if (!_currentUser.Roles.Contains("Gardener"))
            throw new UnauthorizedAccessException("Only gardeners can view their calendar.");
        ...
        var scheduleIds = await _dbContext.TaskScheduleRequests
            .Where(sr => sr.GardenerId == gardenerId)
        ...
        var clientIds = schedules.Select(sr => sr.ClientId).Distinct().ToList();
